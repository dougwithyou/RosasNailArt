...
async function sendAttendanceReminder({ to, clientName, serviceName, startAt, confirmUrl }) {
  const when = new Date(startAt).toLocaleString('es-US', {
    timeZone: 'America/New_York',
    hour: 'numeric', minute: '2-digit',
  });
  return getResend().emails.send({
    from: process.env.RESEND_FROM_EMAIL,
    to,
    subject: `Tu cita es en 30 min — confirma tu asistencia`,
    html: `
      <p>Hola ${clientName},</p>
      <p>Tu cita de <b>${serviceName}</b> es hoy a las <b>${when}</b> en Rosas Nails Art.</p>
      <p><a href="${confirmUrl}" style="background:#a97e2f;color:#fff;padding:12px 24px;border-radius:24px;text-decoration:none;display:inline-block">Confirmar mi asistencia</a></p>
      <p>Si no vas a poder llegar, por favor avísanos cuanto antes por WhatsApp al +1 (571) 513-8905.</p>
      <p>Dirección: 7548 Diplomat Dr B, Suite 201, Manassas, VA 20109</p>
    `,
  });
}
